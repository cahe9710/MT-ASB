function y = sail_dym(w1,w2,w3,w4,AWAvec)
%rigs,results,k]
w1 = single(w1);
w2 = single(w2);
w3 = single(w3);
w4 = single(w4);
wingsAngles = [w1,w2,w3,w4];
%wingsAngles = double(wingsAngles);
AWAvec = double(AWAvec);
%wingsAngles = [w1,w2,w3,w4];
addpath('code');
%clear;
% close all;
...
Nxflap = 2;                % Same for all flaps in this case
Nzall = 6;                 % Same for all flaps and wings, in all cases
...
%         wingsAngles(k,:) = repmat(wingAng,1,4);
        for ii = 1:length(rigs)
            rigs(ii).wing.sheetAngle = deg2rad(wingsAngles(ii)); %k
            rigs(ii).wing.heelAngle = deg2rad(heelAngle);
            rigs(ii).flap.sheetAngle = rigs(ii).wing.sheetAngle;
            rigs(ii).flap.flapAngle = deg2rad(flapsAngles(k,ii)); %k
            rigs(ii).flap.heelAngle = deg2rad(heelAngle);
...
y = [results(1).fx(1)*sin(deg2rad(w1)); results(2).fx(1)*sin(deg2rad(w2)); results(3).fx(1)*sin(deg2rad(w3)); results(4).fx(1)*sin(deg2rad(w4))];
